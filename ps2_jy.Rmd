...
performance_diff_summary_d = performance_diff_d %>%
   group_by(ConfrontationalGame, MusicCondition) %>%
    summarize(MeanImprovementScore = mean(ImprovementScore, na.rm=T),
              semImprovementScore = (sd(ImprovementScore, na.rm=TRUE) / 
               sqrt(length(ImprovementScore[!is.na(ImprovementScore)])))
              ) 


#    summarize(semImprovementScore = (sd(ImprovementScore, na.rm=TRUE) / 
 #              sqrt(length(ImprovementScore[!is.na(ImprovementScore)])))
...
ggplot(performance_diff_summary_d, aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition)) +
  geom_bar(position="dodge", stat="identity") + 
  geom_errorbar(position="dodge",mapping=aes(x=ConfrontationalGame, ymin=MeanImprovementScore+semImprovementScore, ymax=MeanImprovementScore-semImprovementScore), size=0.71, color="grey") +
  scale_fill_brewer(palette="Set1") 
