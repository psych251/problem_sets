...
Highest number: [7]
Lowest number: [1]
** Found that it looks like there were 2 different likert scales and the game scales are 1-7 but the how scales have a range 1-5
...
The researchers have made a column called `DoNotUse` based on their exlusion criteria. Use this column to filter the dataframe! (hint: this is a little trickier than it might seem because of how R treats NA values. You may want to use `?unique` to check values in the column and check out `?is.na`.)
